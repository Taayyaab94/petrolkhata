...
# Every table in TENANT_TABLES currently existing in production belongs to
# this one pump once this migration finishes - there is no signup flow
# yet (that's Stage 2), so every pre-existing row has to land somewhere.
DEFAULT_PUMP_ID = 1
...
def upgrade():
    ...
    # Raw Core SQL, not the ORM - and an explicit id rather than relying
    # on autoincrement/RETURNING (which differ between SQLite and
    # Postgres), which is safe because the pump table is guaranteed empty
    # at this point on every database this migration will ever run
    # against.
    op.execute(
        sa.text(
            "INSERT INTO pump (id, name, created_at) VALUES "
            "(:id, :name, CURRENT_TIMESTAMP)"
        ).bindparams(id=DEFAULT_PUMP_ID, name="My Pump")
    )
    ...
    for table in TENANT_TABLES:
        op.execute(
            sa.text(f"UPDATE {table} SET pump_id = :pid").bindparams(pid=DEFAULT_PUMP_ID)
        )
